	...
	$scope.registervendor = function() {
		...
		if(!$scope.vendor.name) {
			...
			$http({
				...
				headers:{'JWT-AuthToken':localStorage.pmstoken},
				data:{name:$scope.vendor.name, email:$scope.vendor.email,email2:$scope.vendor.email2, phoneno: $scope.vendor.phoneno, phoneno2:$scope.vendor.phoneno2, address:$scope.vendor.address, materialvalues:$scope.vendor.materialvalues, type:$scope.vendor.type, cin:$scope.vendor.cin, tin:$scope.vendor.tin, pan:$scope.vendor.pan, contact_person:$scope.vendor.contact_person, servicetaxno:$scope.servicetaxno, accountdetails:$scope.vendor.accountdetails }
			}).
			success(function(result){
				$rootScope.showloader=false;
				if(result == 0) {
				...
				if(vgoodscount > 0) {
					...
					angular.forEach($scope.submat.level1mat, function(indimat){
						...
						if(indimat.selected){
							...
							$scope.pomateriallistnew[pomatlen]["materials"].push({"materialdesc":indimat.storematerial.name, "uom":indimat.uom.stmatuom.uom, "uomid":indimat.uom.id, "qty":indimat.qtythis, "unitrate":indimat.unitrate,"valueofgoods":indimat.valueofgoods, "materialid":indimat.storematerial.id, "remarks":$scope.addmat.remarks, "qty_per_pole":indimat.qty_per_pole, "wt_per_pole":indimat.wt_per_pole});
							...
							angular.forEach($scope.pomateriallist,function(pomat){
								...
								if(!$scope.pomateriallistnewpre[pomat['storematerial']['parent_id']]) {
									...
									if(pomat['storematerial']['parent_id'] != 0) {
										...
										$scope.pomateriallistnewpre[pomat['storematerial']['parent_id']]['units'] = pomat.storeuom.stmatuom.uom;
								...
								if($scope.pomateriallistnewpre[pomat['storematerial']['parent_id']]['type'] == 3) {
									...
									$scope.pomateriallistnewpre[pomat['storematerial']['parent_id']]['materials'].push({'materialdesc':pomat['storematerial']['name'], 'uom':pomat['storeuom']['stmatuom']['uom'], 'qty':pomat['quantity'], 'unitrate':pomat['unit_rate'], 'valueofgoods':pomat['value_of_goods'], 'materialid':pomat['material_id'], 'freightinsurance_rate':pomat.freightinsurance_rate, 'inspected_quantity':pomat.inspected_quantity, 'approved_quantity':pomat.approved_quantity, 'dispatch_quantity':pomat.dispatch_quantity, 'internal_di_quantity':pomat.internal_di_quantity, 'payment_qty':pomat.payment_qty, 'remarks':pomat.remarks, 'uomid':pomat.store_material_uom_id, 'total_po_qty':pomat['indenttotal']['total_po_qty'], 'total_indent_qty':pomat['indenttotal']['total_indent_qty']});
								}						
...
app.controller("PurchasesCSController",function($scope,$http,$rootScope,$state,Logging, Commas,Dates){
		...
		if(!$scope.enquiry){
			...
			$http({
				...
				if(result != 0 && result != 2) {
					...
					$scope.generatecs = 0;
					console.log($scope.latestindent);
				} else if(result == 0) {
			...
			$http({
				...
				var cs = [
							...
							[//3
								{'field':'','rows':1,'cols':3,'class':'','style':''},
								{'field':'BOQ Quantity','rows':1,'cols':subproj_no,'class':'text-center bold-font','style':'vertical-align:middle;'},
								{'field':'Current Indent Qty','rows':1,'cols':subproj_no,'class':'text-center bold-font','style':'vertical-align:middle;'},
				...
				angular.forEach(csmaterials,function(matinfo,matid){
								...
								if($scope.project.id == 5) {
									...
									for(var k=0;k<$scope.project.subprojects[i]['multiplier'].length;k++){
										...
										if(j==0){
											var y = angular.copy(dummy);
											if($scope.boqdet[matid]) {
												y['field'] = parseFloat($scope.boqdet[matid][$scope.project.subprojects[i]['multiplier'][k]['id']]['qty']).toFixed(2);
											} else {
												y['field'] = 0;
											}
											bdgtrate = parseFloat(boqmaterials[matid]['budget_rate']).toFixed(2);
					...
					if(!$scope.latestindent[matid]) {
						$scope.latestindent[matid] = 0;
					}
					tq.field = parseFloat($scope.latestindent[matid]).toFixed(2);
					totqty = parseFloat($scope.latestindent[matid]).toFixed(2);
					dummyrow.push(tq);
	...
	// 				$("#selectall").prop('checked', false);
	// 				var termsarr= [];
	// 				angular.forEach($scope.enquirydetails['vendors'][0]['quotationterms'],function(thisterms){

	// 					termsarr[thisterms['quotation_default_term_id']] = [];
	// 					termsarr[thisterms['quotation_default_term_id']]['vendors'] = [];

	// 				});

	// 				angular.forEach($scope.enquirydetails['vendors'],function(enqdet){

						
	// 					angular.forEach(enqdet['quotationterms'],function(enqterm){
	// 						termsarr[enqterm['quotation_default_term_id']]['termtitle'] = enqterm['quotationdefault']['termtitle'];
	// 						termsarr[enqterm['quotation_default_term_id']]['vendors'].push({'vendorid':enqdet.id, 'termtitle':enqterm['quotationdefault']['termtitle'], 'termdesc':enqterm.term_desc});

	// 					});
						
	// 				});

	// 				$scope.spterms = termsarr;
	// 				console.log($scope.spterms);
	...
	// 			$scope.selectedvalues.push(enqdet);
	// 		});
	// 	} else {
	// 		$scope.selectedvalues = [];
	// 		$(".childcheckbox").prop('checked', false);
	...
	// 		swal("Please select atleast one vendor to generate the report.");
	// 	} else {
	...
	// 		$http({
	// 			method:'POST',
	// 			url:$rootScope.requesturl+'/get_cs_details',
	// 			data:{selectedvendors:$scope.selectedvalues},
	// 			headers:{'JWT-AuthToken':localStorage.pmstoken},
	// 		}).
	// 		success(function(result){

				
	// 			$scope.csdetails = result;	



	// 			angular.forEach($scope.csdetails,function(csdet){

	// 				var count = 0;
	// 				var least = [];					

	// 				angular.forEach(csdet.enqmaterials,function(csmat){
							
	// 					if(count == 0) {

	// 						least['unit_rate'] = csmat['quotation_taxed_rate'];
	// 						least['id'] = csmat['enquiry_vendor_id'];
	// 					} else {

	// 						if(csmat['quotation_taxed_rate'] < least['unit_rate']) {
	...
	// 				});
	// 				var count2 = 0;
	// 				angular.forEach($scope.selectedvalues,function(selval){
	// 					var check = 0;
	// 					angular.forEach(csdet.enqmaterials,function(csmat){

	// 						if(csmat['enquiry_vendor_id'] == selval.id) {
	...
	// 						csdet.enqmaterials.splice(count2, 0, {});

	// 						csdet.enqmaterials[count2]['quotation_taxed_rate'] = "-";
	// 						csdet.enqmaterials[count2]['quotation_total_taxed'] = "-";
	// 						csdet.enqmaterials[count2]['enquiry_vendor_id'] = selval.id;

	// 					}

	// 					count2++;
	// 				});
	// 				csdet.least_unit_rate = least['unit_rate'];
	// 				csdet.least_id = least['id'];
	// 			});
	// 			console.log($scope.csdetails);

	// 			$http({
	// 				method:'POST',
	// 				url:$rootScope.requesturl+'/get_cs_vendorcosts',
	// 				data:{selectedvendors:$scope.selectedvalues},
	// 				headers:{'JWT-AuthToken':localStorage.pmstoken},
	// 			}).
	// 			success(function(result){

	// 				$scope.vendorcosts = result;
	// 				var count = 0;
	// 				var leastnet = [];

	// 				angular.forEach($scope.vendorcosts,function(vcost){

						
	// 					if(count == 0) {

	// 						leastnet['net_value'] = vcost['net_value'];
	// 						leastnet['id'] = vcost['id'];
	// 					} else {

	// 						if(parseFloat(vcost['net_value']) < parseFloat(leastnet['net_value'])) {
	...
	// 				});
	// 				$scope.vendorcosts.leastnetid = leastnet['id'];

					

	// 				$http({
	// 					method:'POST',
	// 					url:$rootScope.requesturl+'/get_cs_taxes',
	// 					data:{selectedvendors:$scope.selectedvalues},
	// 					headers:{'JWT-AuthToken':localStorage.pmstoken},
	// 				}).
	// 				success(function(result){
	// 					$rootScope.showloader=false;
	// 					console.log(result);
	// 					$scope.cstaxes = result;	

	// 					angular.forEach($scope.cstaxes,function(cstax){
	// 						var count3 = 0;
	// 						angular.forEach($scope.selectedvalues,function(selval){
	// 							var check = 0;
	// 							angular.forEach(cstax.enqtaxes,function(cst){

	// 								if(cst['enquiry_vendor_id'] == selval.id) {
	...
	// 								cstax.enqtaxes.splice(count3, 0, {});

	// 								cstax.enqtaxes[count3]['tax_amount'] = "-";
	// 								cstax.enqtaxes[count3]['enquiry_vendor_id'] = selval.id;
	...
	// $scope.getRemainingTd = function(lendiff) {

	// 	return new Array(lendiff);
	// }
...
app.controller("PurchasesFabMapController",function($scope,$http,$rootScope,$state,Logging, Commas,Dates){
	...
	$scope.savefabmat = function(){
		var countlevel = 0;
		angular.forEach($scope.podet.pomaterials[0]['level1mat'], function(indilevel){

			if(indilevel.selected){
				countlevel++;
			}
		});
